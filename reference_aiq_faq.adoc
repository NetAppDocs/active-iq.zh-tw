...
. 按一下 link:https://mysupport.netapp.com/site/help["請按這裡"] 開啟NetApp支援網站上的「非技術意見回饋工具」。
...
*如何在Active IQ 《Se Digital Advisor》中存取儲存效率？*

. 開啟的首頁 link:https://activeiq.netapp.com/?source=onlinedocs["《數位顧問》 Active IQ"]。
...
*所有的不完整系統都能顯示倍率* Active IQ ONTAP
...
如果您選取「*不含Snapshot*」核取方塊、Active IQ 那麼就會在客戶層級、站台層級、群組層級、叢集層級和節點層級上、計算效率比率（重複資料刪除、壓縮、資料壓縮及複製儲存效率技術的比率）。
...
*用Active IQ 什麼方法計算出功能在《Se Digital Advisor》*中的功能Active IQ 是針對叢集和節點（不包括root和Snapshot複本）來計算
...
*容量顯示於Active IQ 《S16 Digital Advisor Base 2》或《Base 10》嗎？* Active IQ 顯示在《21》中的所有容量均為Base 2（除以1024）、代表GiB/TiB的容量。NetApp產品及其他NetApp產品也會在Base 2中顯示容量使用量。ONTAP
