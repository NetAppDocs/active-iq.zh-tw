---
sidebar: sidebar 
permalink: task_protect_systems_against_ransomware_risks.html 
keywords: ransomware, risks, ransomware risks 
summary:  
---
= 保護Digital Advisor管理的儲存系統免受勒索軟體風險侵害
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
登入 Digital Advisor 時、您可以檢視 * 安全性與勒索軟體防護 * 小工具中反白顯示的 * 行動 * 、其中會顯示風險計數。

您可以檢視快照建立、保留和 ONTAP FPolicy 風險，然後採取行動加以修正。

.步驟
. 登入Digital Advisor。
. 在「安全性與勒索軟體防禦」小工具中選擇 *Actions*。
. 清除 *Security 弱點 * 核取方塊。
. 針對所顯示的風險、請檢查影響層級、並遵循建議的行動。
. 選取 *Unique Risks* 索引標籤，然後選取 *Corrective Action* 欄中的連結。
. 選取「受影響系統」索引標籤，以檢視有風險的系統。
. 選擇建議的補救措施來保護系統。

