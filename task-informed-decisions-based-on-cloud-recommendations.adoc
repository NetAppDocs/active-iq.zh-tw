...
---
= 利用Digital Advisor中的雲端建議分析您的儲存系統健康狀況
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
NOTE: Digital Advisor會引導您進入NetApp Console 來實作建議。
...
建議將符合以下條件的磁碟區遷移到Cloud Volumes ONTAP (CVO)：
...
.步驟
. 從左側窗格中選取 *Cloud Recommendations*。
. 在「遷移」窗格中選擇任意一個連結。
. 選擇 *Migrate to Cloud* 以開啟 *Console*。
...
. 從左側窗格中選取 *Cloud Recommendations*。
. 在「備份和存檔」窗格中選擇任一個連結。
. 選擇 *Backup to Cloud* 以開啟 *Console*。
...
. 從左側窗格中選取 *Cloud Recommendations*。
. 在「災難復原」窗格中選擇任一個連結。
. 選擇 *Replicate to Cloud* 以開啟 *Console*。
