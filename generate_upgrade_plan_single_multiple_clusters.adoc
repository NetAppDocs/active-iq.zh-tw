---
sidebar: sidebar 
permalink: generate_upgrade_plan_single_multiple_clusters.html 
keywords: upgrade advisor, upgrade plan, ontap, upgrade recommendation, clusters 
summary: 提供步驟、為單一叢集和多個叢集成功進行 ONTAP 升級或還原而產生升級計畫。 
---
= 在Digital Advisor中為單一和多個叢集產生ONTAP升級計劃
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
您可以使用 Upgrade Advisor 來檢視符合升級資格或不符合升級資格的叢集清單。您可以檢視合格叢集的升級建議、並產生升級計畫。您可以修正不合格叢集的問題、使其符合升級資格。



[[before-you-begin]]
== 開始之前

在製定升級計劃之前，您應該：

* 請確保 AutoSupport (ASUP) 已啟用並正確配置，且包含所有必要部分。有關更多資訊，請參閱link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["ONTAPAutoSupport解決方案指南"]。
* 驗證 AutoSupport 叢集成員與 SAP 叢集配置是否相符。更多信息，link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["提交一個非技術性案例"] 。


.步驟
您可以依照下列步驟、為單一叢集和多個叢集產生升級計畫：

[role="tabbed-block"]
====
.單一叢集
--
. 在儀表板上，選取 *Upgrade Advisor* 小工具中的叢集數量。
+
image:ua_widget.png["顯示叢集數量的螢幕擷取畫面。"]

+
此時將出現 *Upgrade Advisor ONTAP 頁面。

. 您可以檢視符合資格且不符合資格的叢集清單、以進行升級。
+
image:r_enhanced_ua_toggle.png["顯示增強升級建議程式切換的螢幕擷取畫面已開啟。"]

. 選擇要升級的叢集。*Target OS Version* 欄位顯示建議的目標作業系統版本。您可以點選 image:edit_icon.png["編輯按鈕的影像"] 圖示選擇叢集的其他目標作業系統版本。
+
image:r_ua_select_target_OS_version_single_cluster.png["顯示如何選取目標版本的映像。"]

+
** 您可以在「節點」欄中選擇節點數，以檢視叢集的節點總結。
** 您可以從 *建議動作* 欄中選取 *解決問題*，以修復不符合升級條件的叢集問題，使其符合升級條件。+ image:r_ua_resolve_issue.png["顯示問題類型及修正所需解決方案的影像。"]


. 選擇 *Generate Upgrade Plan*。您將被重新導向至 *Upgrade Recommendation* 頁面。
. 在 * 升級建議 * 頁面上、您可以透過 * 風險顧問 * 標籤、檢視與叢集相關的風險詳細資料。您可以透過「 * 升級前檢查 * 」標籤來檢視升級封鎖程式、升級警告和必要的行動、以及與所選目標作業系統版本相關的新功能和增強功能的相關資訊、請參閱「 * 增強和更新的 ONTAP 功能 * 」標籤。
+
image:r_ua_upgrade_recommendation_page.png["顯示升級建議頁面整體檢視的螢幕擷取畫面。"]

+
** 您可以選取其他目標作業系統版本、並檢視風險摘要、升級前檢查報告、以及與該目標作業系統版本相關的新功能和增強功能資訊。
** 您可以選擇 image:ua_export_icon.png["匯出按鈕"] 圖示，將風險總結匯出至 Excel 工作表。


. 從「升級建議」頁面中選擇「產生升級計畫」。
. 在顯示的快顯視窗中提供詳細資料。
  +
image:ua_generate_single_clusters_plan.png["快顯視窗可填入詳細資料、以提交產生升級計畫的要求。"]
. 選擇 *Generate*。您將被重新導向至 *Reports* 頁面。
. 您可以在 * 報告 * 頁面上下載升級計畫、
+

TIP: 您可以選擇 *View Upgrade Advisor Reports* 進入 *Reports* 頁面。+ image:r_ua_view_reports.png["快照顯示「View Upgrade Advisor」選項的位置，供選取使用。"]



在升級 ONTAP 之後、您應該執行一些工作來確認叢集的就緒性。若要深入瞭解、請參閱 link:https://docs.netapp.com/us-en/ontap/upgrade/task_what_to_do_after_upgrade.html["ONTAP 升級後的處理方式"]。

--
.多個叢集
--
. 在儀表板上，選取 *Upgrade Advisor* 小工具中的叢集數量。
+
image:ua_widget.png["顯示叢集數量的螢幕擷取畫面。"]

+
此時將出現 *Upgrade Advisor ONTAP 頁面。

. 您可以檢視符合資格且不符合資格的叢集清單、以進行升級。
+
image:r_enhanced_ua_toggle.png["顯示增強升級建議程式切換的螢幕擷取畫面已開啟。"]

. 選擇要升級的叢集。*目標作業系統版本* 欄位顯示建議的目標作業系統版本。您可以點選 image:edit_icon.png["編輯按鈕的影像"] 圖示選擇叢集的其他目標作業系統版本。
+
image:r_ua_select_target_OS_version.png["顯示如何選取目標版本的映像。"]

+
** 您可以在「節點」欄中選擇節點數，以檢視叢集的節點總結。
** 您可以從 *建議動作* 欄中選擇 *解決問題*，以修復不符合升級條件的叢集問題，使其符合升級條件。+ image:r_ua_resolve_issue.png["顯示問題類型及修正所需解決方案的影像。"]


. 選擇 *Generate Upgrade Plan*。
. 在顯示的快顯視窗中提供詳細資料。
  +
image:ua_generate_multiple_clusters_plan.png["快顯視窗可填入詳細資料、以提交產生升級計畫的要求。"]
. 選擇 *Generate*。您將被重新導向至 *Reports* 頁面。
. 您可以在 * 報告 * 頁面上下載升級計畫、
+

TIP: 您可以選擇 *View Upgrade Advisor Reports* 進入 *Reports* 頁面。+ image:r_ua_view_reports.png["快照顯示「View Upgrade Advisor」選項的位置，供選取使用。"]



在升級 ONTAP 之後、您應該執行一些工作來確認叢集的就緒性。若要深入瞭解、請參閱 link:https://docs.netapp.com/us-en/ontap/upgrade/task_what_to_do_after_upgrade.html["ONTAP 升級後的處理方式"]。

--
====